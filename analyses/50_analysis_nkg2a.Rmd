...
adata_de.obs["nkg2a_status2"] = [
    "pos"
    if clus in ["T CD8+ 3", "T CD8+ 7", "T CD8+ 5", "T CD8+ 9", "T CD8+ 10"]
    else "neg"
    for clus in adata_de.obs["cluster"]
]
adata_de.obs["nkg2a_status_expr"] = [
    "pos" if expr > 0.5 else "neg"
    for expr in adata_de.X[:, adata_de.var_names == "KLRC1"].todense().A1
]
```
...
sc.pl.umap(
    adata_de,
    color=["KLRC1", "nkg2a_status", "nkg2a_status2", "nkg2a_status_expr"],
    size=10,
)
```
...
obs = adata_de.obs.loc[
    :, ["nkg2a_status", "nkg2a_status2", "nkg2a_status_expr", "mt_frac", "n_genes"]
]
